...
ORT_GENAI_QNN_CTX = "onnxruntime-genai + QNN context binaries"
...
def _read_soc_model(path: Path) -> str | None:
    """Pull the qnn soc_model out of genai_config.json, wherever it is nested.

    Pipeline models put provider_options inside each stage rather than on the
    top-level decoder, so search the whole document.
    """
    ...
    def walk(node: object) -> None:
        if isinstance(node, dict):
            for key, value in node.items():
                if key == "qnn" and isinstance(value, dict) and "soc_model" in value:
                    found.append(str(value["soc_model"]))
                walk(value)
...
def classify(model_dir: str | Path) -> ModelFormat:
    ...
    if "genai_config.json" in names:
        ctx_files = _matches(names, "*_ctx.onnx")
        if ctx_files:
            # EPContext wrappers hold a QNN context binary compiled ahead of
            # time for one HTP architecture. They load only on a matching
            # chipset; elsewhere QNN declines the node and ORT falls back to
            # CPU, where the graph has no real weights and fails on shapes.
            soc = _read_soc_model(path)
            soc_note = f" It declares soc_model {soc}." if soc else ""
            return ModelFormat(
                kind=ORT_GENAI_QNN_CTX,
                path=path,
                loadable_by_ort_genai=True,
                summary=(
                    "ONNX Runtime GenAI model with pre-compiled QNN context "
                    "binaries (EPContext). Loadable in principle, but the "
                    "binaries are ahead-of-time compiled for a specific "
                    f"Hexagon architecture.{soc_note}"
                ),
                remedy=(
                    "Confirm the target chipset matches yours. On a mismatch the "
                    "load fails with 'EPContext node generated by "
                    "QNNExecutionProvider is not compatible'. soc_model 60 is "
                    "Snapdragon X Elite (HTP v73); 88 is X2 Elite (HTP v81)."
                ),
                evidence=["genai_config.json", *ctx_files[:3]],
            )
        return ModelFormat(
            kind=ORT_GENAI,
            path=path,
